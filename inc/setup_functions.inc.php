...
function hesk_testSMTP($check_old_settings=false)
{
    ...
    // We can't use both SSL and TLS
    if ($set['smtp_ssl']) {
        $set['smtp_host_name'] = 'ssl://' . $set['smtp_host_name'];
        $set['smtp_tls'] = 0;
    }

    try {
        //Connect to an SMTP server
        if (!$smtp->connect($set['smtp_host_name'], $set['smtp_host_port'])) {
        ...
        if (is_array($e) && array_key_exists('AUTH', $e)) {
            if ($smtp->authenticate($set['smtp_user'], hesk_htmlspecialchars_decode(stripslashes($set['smtp_password'])))) {
                echo 'Connected ok!';
            } else {
